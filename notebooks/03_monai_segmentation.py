...
# %% Import helpers
from src.loaders.monai import recommend_transforms, to_monai_dict_dataset
...
if _MONAI_AVAILABLE and _TORCH_AVAILABLE and data_dicts:
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for batch_i, batch in enumerate(monai_loader):
            ...
            if batch_i % 5 == 0:
                print(
                    f"  Epoch {epoch + 1} batch {batch_i:3d}/{len(monai_loader)} -- Dice loss: {loss.item():.4f}"
                )

        print(
            f"Epoch {epoch + 1}/{NUM_EPOCHS} -- avg loss: {epoch_loss / max(len(monai_loader), 1):.4f}"
        )
