...
def build():
    ...
    pdf.cell(0, 7, "A clinical brain MRI dataset",
    ...
    # Hero image
    pdf.img(str(ROOT / "output/000_s0007_Ax_T2_FLAIR/s0007_Ax_T2_FLAIR_multiplane.png"),
            None, w=178)

    pdf.set_y(-22)
    pdf.set_font("Helvetica", "", 8)
    pdf.set_text_color(*GREY)
    pdf.cell(0, 4, safe(f"{period.get('start','')} to {period.get('end','')}  -  India"),
             align="C")
    ...
    pdf.table(["Vendor", "Model", "Field", "Files", "Share"], rows,
              [22, 60, 18, 45, 45], aligns=["L", "L", "C", "R", "R"])
    ...
    pdf.table(["Series description", "Count"], seq_rows,
              [140, 50], aligns=["L", "R"])
    ...
    pdf.table(["Indication", "Files"], case_rows,
              [140, 50], aligns=["L", "R"])
    ...
    if study:
        ...
        pdf.table(["#", "Description", "Type", "Files", "TR/TE", "Grade"],
                  rows,
                  [16, 64, 38, 22, 30, 20],
                  aligns=["L", "L", "L", "R", "R", "C"])
    ...
    cases = [
        ("Stroke and acute care",
         "DWI / ADC pairs at scale with paired structural sequences."),
        ...
        ("Quality assessment",
         "Pre-graded so quality models can train against reference labels."),
