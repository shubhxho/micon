...
from .helpers import safe_squeeze, get_2d_slice
...
def _extract_plane_slices(vol_norm: np.ndarray, plane: str,
                          n_per_plane: int) -> tuple[str, int, list[np.ndarray], np.ndarray]:
    """Extract slices for a single plane — numpy only, GIL-releasing."""
    nz, ny, nx = vol_norm.shape[:3]
    if plane == "Axial":
        n = nz
        indices = np.linspace(max(n // 8, 0), min(n - n // 8, n - 1), n_per_plane, dtype=int) if n > 1 else np.array([0])
        slices = [get_2d_slice(vol_norm, i) if i < nz else np.zeros((ny, nx)) for i in indices]
    elif plane == "Coronal":
        n = ny
        indices = np.linspace(max(n // 8, 0), min(n - n // 8, n - 1), n_per_plane, dtype=int) if n > 1 else np.array([0])
        slices = [vol_norm[:, min(i, ny - 1), :] if vol_norm.ndim >= 3 else vol_norm[0] for i in indices]
    else:  # Sagittal
        n = nx
        indices = np.linspace(max(n // 8, 0), min(n - n // 8, n - 1), n_per_plane, dtype=int) if n > 1 else np.array([0])
        slices = [vol_norm[:, :, min(i, nx - 1)] if vol_norm.ndim >= 3 else vol_norm[0] for i in indices]
    return plane, n, slices, indices


def export_multiplane_montage(vol: np.ndarray, series_name: str, out_dir: str,
                               n_per_plane: int = 6,
                               vol_stats: dict | None = None) -> str:
    """Export multiplane montage using PIL — no matplotlib lock, fully parallel."""
    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    vol = safe_squeeze(vol)

    from .metal import gpu_normalize, gpu_available
    ...
    with ThreadPoolExecutor(max_workers=3) as pool:
        futs = [pool.submit(_extract_plane_slices, vol_norm, p, n_per_plane)
    ...
    grade_colors = {"A": (76, 175, 80), "B": (139, 195, 74), "C": (255, 193, 7),
                    "D": (255, 87, 34), "F": (244, 67, 54)}
    ...
    if grade and grade in grade_colors:
        ...
        draw.text((badge_x + 22, 30), grade, fill=(255, 255, 255),
                  font=font_grade, anchor="mm")

    draw.text((total_w // 2, 10), display_name, fill=(0, 220, 240),
    ...
    if vol_stats:
        ...
        stats = (f"Volume: {nz}x{ny}x{nx}  |  "
                 f"Tissue: {vol_stats.get('volume_tissue_pct', 0):.0f}%  |  "
                 f"Entropy: {vol_stats.get('volume_entropy', 0):.1f} bits")
        draw.text((total_w // 2, 34), stats, fill=(139, 148, 158),
                  font=font_small, anchor="mt")
        draw.text((total_w // 2, 50), f"SNR: {snr:.1f}", fill=snr_color,
                  font=font_small, anchor="mt")
    ...
    for row, (plane_name, n_total, slices, indices) in enumerate(plane_defs):
        for col, slc in enumerate(slices[:n_per_plane]):
            ...
            slice_meta.append((row, col, plane_name, n_total, indices[col] if col < len(indices) else 0))
    ...
    for (row, col, plane_name, n_total, idx), pil_img in zip(slice_meta, pil_imgs):
        y_base = header_h + row * (cell_h + pad + 16)
        if col == 0:
            draw.text((4, y_base + cell_h // 2), plane_name, fill=(0, 255, 255),
                      font=font_label, anchor="lm")
        x = label_w + col * (cell_w + pad)
        canvas.paste(pil_img.convert("RGB"), (x, y_base))
        draw.text((x + cell_w // 2, y_base + cell_h + 2),
                  f"{idx}/{n_total}", fill=(170, 170, 170),
                  font=font_small, anchor="mt")
...
def _draw_bar_chart(
    draw: ImageDraw.Draw, counts: np.ndarray, edges: np.ndarray,
    x0: int, y0: int, w: int, h: int,
    bar_color: tuple[int, int, int], log_scale: bool,
    pcts: np.ndarray | None, title: str,
    font: ImageFont.FreeTypeFont, font_sm: ImageFont.FreeTypeFont,
    stats_text: str | None = None,
) -> None:
    """Draw a single histogram panel on a PIL canvas."""
    # Background with subtle rounded border
    draw.rounded_rectangle([x0, y0, x0 + w, y0 + h], radius=6, fill=(13, 17, 23),
                           outline=(30, 36, 44), width=1)
    ...
    for i, v in enumerate(vals):
        ...
        by = plot_y + plot_h - bh
        draw.rectangle([bx, by, bx + max(int(bar_w), 1), plot_y + plot_h],
                       fill=bar_color)

    # Axes
    draw.line([plot_x, plot_y, plot_x, plot_y + plot_h], fill=(48, 54, 61), width=1)
    draw.line([plot_x, plot_y + plot_h, plot_x + plot_w, plot_y + plot_h], fill=(48, 54, 61), width=1)

    # Y-axis labels
    ylabel = "Log Count" if log_scale else "Count"
    draw.text((x0 + 4, plot_y + plot_h // 2), ylabel, fill=(139, 148, 158),
              font=font_sm, anchor="lm")
    ...
    if stats_text:
        ...
        for line in stats_text.split("\n"):
            draw.text((x0 + w - pad_r - 4, sy), line,
                      fill=(139, 148, 158), font=font_sm)
            sy += 12


def export_histogram(vol: np.ndarray, series_name: str, out_dir: str,
                     vol_stats: dict | None = None) -> str:
    ...
    hist_counts, hist_edges, pcts = gpu_histogram_and_percentiles(
        flat, bins=200, pcts=[5, 25, 50, 75, 95],
    )
    ...
    if vol_stats:
        ...
        draw.text((total_w // 2, 32), stats_line, fill=(100, 110, 120),
    ...
    _draw_bar_chart(draw, hist_counts, hist_edges,
                    pad, header_h, panel_w, panel_h,
                    (33, 150, 243), False, pcts, "Linear Scale",
                    font, font_sm)
    _draw_bar_chart(draw, hist_counts, hist_edges,
                    panel_w + pad * 2, header_h, panel_w, panel_h,
                    (255, 152, 0), True, pcts, "Log Scale",
                    font, font_sm, stats_text)
...
def export_enhanced_views(vol: np.ndarray, series_name: str, out_dir: str,
                          vol_stats: dict | None = None) -> str | None:
    ...
    display_name = series_name.replace("_", " ")
    draw.text((total_w // 2, 8), f"{display_name} — Enhanced Views",
              fill=(0, 220, 240), font=font_title, anchor="mt")
    ...
    if grade and grade in grade_colors:
        ...
        draw.text((badge_x + 20, 24), grade, fill=(255, 255, 255),
                  font=fonts["grade"], anchor="mm")

    if vol_stats:
        info_text = (f"Volume: {nz}x{ny}x{nx}  |  "
                     f"SNR: {vol_stats.get('volume_snr_estimate', 0):.1f}  |  "
                     f"Tissue: {vol_stats.get('volume_tissue_pct', 0):.0f}%")
        draw.text((total_w // 2, 32), info_text, fill=(139, 148, 158),
                  font=font_small, anchor="mt")
    ...
    for row_idx, (row_label, projections, col_labels) in enumerate(rows_data):
        ...
        # Row label
        draw.text((4, y_base + cell_h // 2), row_label,
                  fill=(0, 255, 255), font=font_label, anchor="lm")
        ...
        for col_idx in range(3):
            ...
            if row_label == "Tissue":
                ...
                while mask_u8.ndim > 2:
                    mask_u8 = mask_u8[0]
                rgb = np.stack([base, np.clip(base.astype(np.int16) + mask_u8, 0, 255).astype(np.uint8), base], axis=-1)
                pil_img = Image.fromarray(rgb, mode="RGB").resize((cell_w, cell_h), Image.LANCZOS)
            else:
                proj = projections[col_idx]
                while proj.ndim > 2:
                    proj = proj[0]
                pil_img = Image.fromarray(_norm_to_uint8(proj), mode="L").resize(
                    (cell_w, cell_h), Image.LANCZOS).convert("RGB")

            canvas.paste(pil_img, (x, y_base))
            draw.text((x + cell_w // 2, y_base + cell_h + 2), col_labels[col_idx],
                      fill=(170, 170, 170), font=font_small, anchor="mt")
